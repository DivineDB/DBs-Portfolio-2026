...
import { LayoutTemplate, MousePointer2, Cpu, BrainCircuit, Globe, ArrowDown, Download, Link2, Mail } from "lucide-react";
...
const TECH_CATEGORIES: {
  ...
  },
  {
    ...
    tools: [
      ...
      { label: "Information Architecture", Icon: LayoutTemplate, color: "#8B5CF6" },
    ],
  },
  {
    id: "core",
...
// ─── Closing quote ────────────────────────────────────────────────────────────
const QUOTE_WORDS = "So, are you ready to make me an offer I can't refuse?".split(" ");
...
function TechCard({
  ...
  return (
    <div
      ...
      style={{
        backgroundImage: "radial-gradient(rgba(42, 71, 86, 0.03) 1px, transparent 1px)",
    ...
    >
      <div>
        ...
        <div className="flex items-center gap-3 pb-4 mb-4 border-b border-dashed border-[#2A4756]/10">
          ...
          <h3 className="font-gilroyBold text-lg tracking-tight text-text_primary">{title}</h3>
        </div>
        ...
        <ul className="space-y-3 relative z-10">
          {tools.map(({ label, Icon, color }) => (
            ...
            >
              ...
              />
              <span className="font-gilroyRegular text-sm text-text_primary/80">{label}</span>
            </motion.li>
          ))}
        </ul>
      </div>
      ...
      <section id="top" className="relative min-h-screen px-6 md:px-12 py-24 md:py-0 flex items-center">
        <MotionInView className="relative mx-auto flex h-full w-full max-w-[1000px] flex-col justify-center">
          ...
          <p className="mt-5 max-w-2xl text-base leading-relaxed text-slate-700 md:text-lg">
            If you&apos;re a recruiter or building a team with an opportunity I simply can&apos;t
            refuse—you&apos;re in the right place.
          </p>
